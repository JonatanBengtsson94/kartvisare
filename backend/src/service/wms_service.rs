...
impl<R: WmsRepository> WmsService<R> {
    ...
    pub async fn get_wms(&self, id: i32) -> Result<Option<WmsDetails>, sqlx::Error> {
        let wms = self.repository.get_wms(id).await?;
        Ok(wms)
    }
...
mod tests {
    ...
    async fn test_get_wms_details_found() {
        ...
        mock_repo.expect_get_wms().with(eq(1)).returning(|_| {
            Ok(Some(WmsDetails {
                id: Some(1),
        ...
        let details = service.get_wms(1).await.unwrap();
    ...
    async fn test_get_details_not_found() {
        ...
        let details = service.get_wms(999).await.unwrap();
